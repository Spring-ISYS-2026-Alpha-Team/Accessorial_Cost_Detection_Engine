...
Target variable:
    ...
    had_accessorial (1 = shipment incurred any accessorial charge)



Output:

    risk_score (0–1 probability), risk_tier (Low/Medium/High)
...
def _train_model(df: pd.DataFrame):
    ...
    df = df[present_needed].dropna(subset=[_TARGET])



    if len(df) < 40:

        raise ValueError(f"Need at least 40 rows to train — only {len(df)} rows available.")
    ...
    opt_medium = min(opt_medium, opt_high - 0.05)
...
def get_risk_model():
    ...
    - If not found, return (None, error metadata).



    This avoids retraining every time the Streamlit app loads.
    ...
    return None, {"error": "No trained risk model found on disk."}





def train_and_save_if_missing(df: pd.DataFrame):
    ...
    - Otherwise train once, save it, and return it.



    Use this only when you explicitly want first-run bootstrap training.
    ...
    if weight > avg_wt * 1.35:
        ...
        factors.append(
            ...
            (
                ...
                f"{weight:,.0f} lbs is well above fleet avg of {avg_wt:,.0f} lbs",
        ...
        factors.append(
            ...
            (
                ...
                f"{weight:,.0f} lbs is well below fleet avg of {avg_wt:,.0f} lbs",
    ...
    if month in (10, 11, 12):
        ...
        factors.append(
            ...
            (
                ...
                "Oct–Dec capacity crunch raises accessorial incidence by 15–25%",
        ...
        if f_avg is not None and f_avg > fleet_avg * 1.2:
            ...
            factors.append(
                ...
                (
                    ...
                    f"{facility} averages {f_avg:.0%} risk — above fleet avg",
